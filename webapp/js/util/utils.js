...
/**
 * desabilitar e voltar o estado anterior dos elementos especificado pela tag 
 * útil para bloquear a interação do usuário com a tela enquanto algo estiver processando, ou simplesmente para travar
...
HabilitaDesabilitaElementos.prototype = {
	 ...
	 * @param {String} idelemento elemento pai, opcional, se não passado desabilitará todas as tags especificadas do documento
	 */
	...
	ativo: true,
	/**
	 * troca o estado dos elementos desabilitados, utilize essa função para voltar o estado normal dos objetos desativados
	 */
	doEstado: function(){
		var selects = null;
...
CheckboxFunctions = {
	/**
	 * marca todos os checkbox onde o valor do mesmo seja igual a um dos valores do array de integer passado no argumento
	 * 
	 ...
	 * @param {String} propertyName nome da propriedade dentro dos objetos do array que será extraído o valor a ser marcado (opcional) 
	 */
	 ...
	 * @param {String} value propriedade contida nos elementos do arraydata que possui o valor dos checks
	 * @param {String} text propriedade contida nos elementos do arraydata que possui o label dos checks
...
RadioFunctions = {
	 ...
	 * @param {String} value propriedade contida nos elementos do arraydata que possui o valor dos radios
	 * @param {String} text propriedade contida nos elementos do arraydata que possui o label dos radios
	...
	/**
	 * set um valor em um RADIO
	 * deve ser utilizado no lugar do dwr.util.getValue quando o objeto passado não é o id de um objeto
	 * ou seja o seguinte código não funcionaria com o dwr.util
	 * <code>
...
ComboFunctions = {
	/**
	 * transfere um indice selecionado em um combo para outro combo
	 * o valor dos options a serem manipulados devem conter apenas números
	 *
	 * @param {String} origem id do objeto combo que possui um índice selecionado
	 * @param {String} destino id do objeto combo destino onde o option representado pelo índice selecionado será adicionado 
	...
	/**
	 * transferir todos ostions de um select para outro
	 * o valor dos options a serem manipulados devem ser sempre números
	 * pois não será transferido os que já estiverem no select destino (que possuirem o mesmo value)
	 *
	 * @see dwr.util.addOptions
	 ...
	 * substituida no dia 13/03/2008 pois o sistema deve selecinar e enviar so as selecionadas 
	 */
	transfereTodosOld: function(origem, destino){
		...
		if(Prototype.Browser.IE){
			...
			//TODO: Existe um BUG no firefox versão 2.0.0.3 que as vezes o dwr.util.removeAllOptions não funciona
			dwr.util.removeAllOptions(origem);
		}
	},
	
	/**
	 * transferir  options de um select para outro
	 * o valor dos options a serem manipulados devem ser sempre números
	 * pois não será transferido os que já estiverem no select destino (que possuirem o mesmo value)
	 *
	 * @see dwr.util.addOptions
	...
	/**
	 * pegar todos os valores selecionados em um select-multiple
	 * a função considera que o valor dos options contenham apenas números interos
	 * 
	...
	/**
	 * fecha a janela de pesquisa, transfere somente as opções selecionadas que não estão no combo
	 *  da tela principal e remove o que foi selecionado das opções disponíveis da tela principal
	 * 
	 ...
	 * @param {String} sel3 select que contém todas as opções na tela principal, onde será removido quando selecionado
	 */
 ...
 * 		})
 * </code>
 * ou então no final da página antes do campo (o que não é aconselhável, por questões de centralização 
 * do código dentro do .js)
 * 
 * então po
 * <b>#</b> - somente números
 * <b>*</b> - números e letras (qualquer caracter)
 * <b>x</b> - somente letras 
 *
 * datas: ##/##/#### ou ##/##/#### ##:##
 * cpf: ###.###.###-##
 * cnpj: ##.###.###/####-##
...
/**
 * Transformar o form em um array de elementos, utilizado para converter o form e enviar para o DWR como objeto.
 * Ele não converte select-multiple, para pegar os values de select-multiple use a função ComboFunctions.getValues(id)
 * e atribua o resultado a uma propriedade do form (i.e. form.instituicoes = ComboFunctions.getValues(id))
 * Use-o ao invés de usar o dwr.util.getValues(form), pois o mesmo apresenta vários bugs
 *
...
/**
 * Conta a quantidade de caracteres passada pelo txtArea, e mostra um mensagem dentro 
 * de um alert para informar ao usuário que a quantidade de caracteres foi extrapolada.
 * 
...
/**
 * formata data fornece a mascara conforme o valor for digitado em evento onkeyup ou press e invoca uma 
 * outra funcao filtra campo para a remocao caso os caracteres especiais tenha sido digitados 
 */
...
/**
 * Aplicar bloquei de tela com mensagem de confirmação de saída no caso 
 * do usuário ter alterado algum campo da tela
 * @param {HTMLFormElement} form
 */
...
/**
 * as funções setPreHook e setPostHook serão chamadas automaticamente pelo DWR, quando começar e terminar
 * as requisições feitas por ele, assim é possível controlar o loading do sistema, mostrando ou escondendo.
 * O mesmo é feito para as requisições feitas com a API do prototype
 */
Event.observe(window, "load", function(){
	...
	dwr.engine.setErrorHandler(function(message, ex){
		...
		}else if(ex.name == "br.gov.bcb.rdr.apresentacao.util.ajaxUtils.SessionExpiredException"){
			JanelasComuns.sessaoFinalizada();
		}else{
