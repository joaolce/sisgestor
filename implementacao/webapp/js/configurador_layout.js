...
function configuraLabel(id) {
	...
	for ( var i = 0; i < labels.length; i++) {
		...
		if ((label.firstChild.nodeName == "INPUT")
		   && ((label.firstChild.type == "radio") || (label.firstChild.type == "checkbox"))) {
...
Event.observe(window, "load", function() {
	...
	if (Prototype.Browser.IE && (window.navigator.appVersion.indexOf("MSIE 6.0") != -1)) {
		...
		document.styleSheets[0].addRule("input",
		   "margin-top: 2px;color: #003366;font-family: verdana;color: #003366;font-size:85%;")
	}
});
