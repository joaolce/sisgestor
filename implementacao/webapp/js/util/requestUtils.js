 ...
 * 
 * <b>(1) para submeter um form assíncronamente para o Struts:</b>
 * <code>
 * 		<form name="exemplo" action="teste.do" onsubmit="requestUtils.submitForm(this); return false;">...</form>
 * </code>
 *
 * <b>(2) para fazer uma requisição GET a uma url e receber o resultado dela:</b>  
 * <code>
...
RequestUtils.prototype = {
	...
	/**
	 * status da requisição se true as mensagens aparecerão em verde, false as mensagens aparecerão em vermelho
	 * @type Boolean
	 */
	status: false,
	...
	/**
	 * @type String nome do campo que será focalizado quando o usuário clicar ok na janela de mensagens
	 */
	focusControl: null,
	/**
	...
	/**
	 * @type String id gerado do objeto, utilizado geralmente para retornar ao Javascript o id do objeto que foi inserido, 
	...
	/**
	 * mostrar uma mensagem de erro quando o servidor cair ou ficar sem 
	 * conexão de internet ou alguma resposta diferente de 200 for recebida
	 * 
	...
	formSubmetido: null,
	/**
	 * fazer uma requisição assíncrona simples utilizando o método GET a uma 
	 * url que deverá responder um XML para o javascript processar
	 * 
	 * @param {String} url
	 * @param {Function} posFunction função a ser executada quando a resposta for recebida (opcional)
	 * @param {Function} posClickFunction função a ser executada quando o usuário clicar ok na janela de mensagens
	 */
	...
	submitForm: function(form, posFunction, posClickFunction){
		if(form == null ||form == undefined || form.nodeName == undefined || form.nodeName != "FORM")
	...
	/**
	 * mostrar as mensagens recebidas do servidor em uma janela própria para mensagens
	 * que até 200px ela se ajustará ao tamanho e a quantidade de mensagens e a partir
	 * desse tamanho ela terá uma barra de rolagem
	 */
	showMessages: function(){
		...
		var img;
		if(this.status){
			img = Builder.node("img", {src: this.imagemSucesso, height: 16, width: 16});
		}else{
			img = Builder.node("img", {src: this.imagemFalha, height: 22, width: 22});
		}
		Element.setStyle(img, {
			marginRight: "5px",
			marginLeft: "3px"
		});
		$("idValidacoes").appendChild(Builder.node("div",{style: "float: left;"}, [img]));
		var div = Builder.node("div", {id: "validations", style: "float: right; width: 550px;"});
		...
		var div2 = Builder.node("div", {align: "center", style: "clear: both;"}, [
			Builder.node("br"),
			Builder.node("input", {type: "button", value: "OK", style: "width: 120px;", id: "buttonValidations"})
		]);
	...
	/**
	 * fechar janela de mensagens e redirecionar o usuário para a url recebida como resposta (se houver)
	 */
	...
	getElementosFocus: function(){
		var elementFocus = $(this.formSubmetido).select("[name=\""+this.getNomeElementoFocus()+"\"]");
