 ...
 * Wrapper de tabelas
 * 
 * Adiciona a todas as linhas de uma tabelas eventos como onclick ou ondblclick além fornecer informações
 * como a referência da linha selecionada (TR) ou o índice da linha selecionada
 * 
...
Tabela.prototype = {
	...
	_tabela: null,
	/**
	 * indica se a tabela é selecionável ou não (selecionável no sentido de clicar segurar o mouse e selecionar o 
	 * texto da tabela) o que não é útil quando a intenção da tabela é apenas clicar em uma linha
	...
	aplicarEventos: function(elemento){
		...
		for(var index=0; index<elemento.childNodes.length; index++) {
			...
			try{
				...
				if(el.firstChild != null && ["span", "div", "td"].include(el.firstChild.nodeName.toLowerCase())){
	...
	_getRewindButton: function(){
		...
		if(resultado.length != 0){
			...
			botao = Builder.node("a", {href: "#", id: "voltarPagina"}, [
				document.createTextNode(" Anterior ")
			]);
			Event.observe(botao, "click", this.rewindResults.bind(this));
			
			var botaoPrimeira = Builder.node("a", {href: "#", id: "ultimaPagina"}, [
			...
			if(this.divPaginacao.childNodes.length == 0){
				...
				this.divPaginacao.insert({top: span});
			}
		}
	...
	_getForwardButton: function(){
		...
		if(resultado.length != 0){
			...
			botao = Builder.node("a", {href: "#", id: "avancarPagina"}, [
				document.createTextNode(" Próximo ")
			]);
			Event.observe(botao, "click", this.forwardResults.bind(this));
			
			var botaoUltimo = Builder.node("a", {href: "#", id: "ultimaPagina"}, [
	...
	setTotalRegistros: function(totalRegistros){
		...
		this.divPaginacao.firstDescendant().insert({after: divNumeracao});
		...
		if(this.totalPaginas > this.qtdPaginasExibidas){
			...
			maximo = this.paginaAtual+(this.qtdPaginasExibidas-(this.paginaAtual%this.qtdPaginasExibidas));
		...
		for(; index<=maximo; index++) {
			...
			if(index == this.paginaAtual+1){
				ancora = Builder.node("span", {style: "background-color: #EFEFEF; color: #0065FF; border: 1px solid #0065FF"}, [
					document.createTextNode(numeracao)
				]);
			}else{
				ancora = Builder.node("a", {href: "#"}, [
					document.createTextNode(numeracao)
				]);
	...
	semRegistros: function(mensagem){
		...
		var trVazia = Builder.node("tr", null, [
			Builder.node("td", {colspan: colunas, align: "center"}, [document.createTextNode(mensagem)])
		]);
	...
	/**
	 * Desabilitar a seleção de um texto da tabela, não permite que o usuário selecione um texto 
	 * da tabela, usado internamente
	...
	/**
	 * habilitar a seleção do elemento usada internamente para não permitir que o usuário 
	 * selecione um texto da tabela
	 * @return{Boolean}
	 */
	_disable : function () {
		return true;
