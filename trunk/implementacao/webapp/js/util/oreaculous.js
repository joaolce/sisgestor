...
Janela.prototype = {
	...
	/**
	 * Indicar se a janela está modal ou não, ao chamar undoModal se ele for chamado novamente não poderá 
	 * fazer nada se o valor do modal for false
	 * @type Boolean
	 */
	modal: true,
	...
	/**
	 * ID do div de dentro da janela que irá conter o conteúdo, também usado para identificar a janela.
	 * Não será possível ter duas janelas com o mesmo ID
	 */	
	idConteudo: null,
	...
	/**
	 * Botão fechar da janela, quando o botão é removido ele vem pra cá para depois poder ser recolado
	 * @type HTMLDivElement
	 */
	botaoFechar: null,
	...
	/**
	 * Manipulador de evento que deverá ser acionado quando o conteúdo de uma URL tiver
	 * sido carregado dentro da div de conteúdo
	 * @type Function
	 */
	...
	/**
	 * Se o método de requisição é POST, aqui conterá a querystring que deverá ir no corpo da requisição POST
	 * @type String
	 */
	postBody: null,
	 ...
	 * @param {Integer} left quantidade de pixels a partir da esquerda do elemento contedor (coordenada X da tela)
	 * @param {Integer} top quantidade de pixels a partir do top do elemento contedor {coordenada Y da tela}
	...
	criarJanela: function(){
		var janelaDiv = Builder.node("div", {UNSELECTABLE: "on", style: this.getEstiloJanela()});
		var divBarra = Builder.node("div",{style: this.getEstiloBarra() }, [
			Builder.node("div",{style: this.getEstiloBarraTitulo(), UNSELECTABLE: "on"}, [document.createTextNode(this.titulo)]),
			Builder.node("div", {style:this.getEstiloDivFechar()}, [
				Builder.node("a", {href: "javascript:;", style: this.getEstiloBotaoFechar()}, [document.createTextNode("X ")])
			])
		]);
		Event.observe(divBarra.lastChild.firstChild, "click", (function(event){
			this.fecharJanela();
		}).bind(this));
		Event.observe(divBarra.lastChild.firstChild, "mousedown", (function(event){
			this.fecharJanela();
		}).bind(this));
		...
		Event.observe(divBarra.firstChild, "mouseup", (function(event){
			if(this.draggable != null){
				...
				/* acontece um BUG com o IE se o usuário clica com o botão do meio do mouse 
				 * em cima da barra de título: a janela fica por trás de tudo */
		...
		janelaDiv.appendChild(Builder.node("div", {id: this.idConteudo, style: this.getEstiloConteudo()}));
	...
	/**
	 * retorna o estilo personalizado pelo usuário com o estilo de 
	 * configuração padrão da barra da janela
	 * @return {String}
	 */
	getEstiloBarra: function(){
		return Configuracao.EstiloBarra.concat("cursor:move; white-space: nowrap;padding-left: 5px; height: 16px;");
	},
	
	/**
	 * retorna o estilo personalizado pelo usuário com o estilo de configuração padrão da barra de janela
	 * @return {String}
	 */
	...
	setHeight: function(height){
		if(height == null){
			Element.setStyle(this.janelaDiv, {height: ""});
		}else{
			this.height = height;
			Element.setStyle(this.janelaDiv, {height: this.height});
		}
	},
	setWidth: function(width) {
 		if(width == null){
			Element.setStyle(this.janelaDiv, {width: ""});
		}else{
			this.width = width;
			Element.setStyle(this.janelaDiv, {width: this.width});
		}
 	},
	/**
	...
	/**
	 * bloqueia os elementos que estão por tráz da janela que foi aberta, é sempre bloqueado
	 * o resto da página se for a primeira janela a ser aberta ou a janela anterior a que acabou 
	 * de ser aberta
	 */
	makeModal: function(){
		var cobertor = null
		if(Prototype.Browser.IE && window.navigator.appVersion.indexOf("MSIE 6.0") != -1){
			cobertor = $(Builder.node("iframe", {frameBorder: "no", scrolling: "no", id:this.idElementoCobertor}));
		}else{
			cobertor = $(Builder.node("div", {id:this.idElementoCobertor}));
		}
	...
	/**
	 * set a janela anterior a que foi acabou de ser aberta, se for a primeira
	 * a propriedade é nula
	 */
	...
	/**
	 * seta o método a ser utilizado na requisição POST ou GET
	 * (padrão: GET)
	 */
	...
	/**
	 * corrige posição da div se a página tiver barras de rolagem verticais, deixando sempre
	 * a janela no centro da tela verticalmente
	 */
	corrigirPosicaoDiv: function(){
		var div = this.janelaDiv;
		if((document.body.clientHeight>document.documentElement.clientHeight && document.documentElement.scrollTop != 0) || (document.body.clientHeight < (div.offsetTop + div.offsetHeight))){
			if(document.body.clientHeight>document.documentElement.clientHeight && document.documentElement.scrollTop != 0){
				if(document.documentElement.clientHeight > div.offsetHeight){
					var tamanhoDivModificado = ((document.documentElement.scrollTop + (document.documentElement.clientHeight / 2)) - (div.offsetHeight /2))+ "px";
					Element.setStyle(div, {top: tamanhoDivModificado}); 
				}else{
				...
				if(document.body.clientHeight > div.offsetHeight){
					var tamanhoDivModificado = ((document.body.clientHeight / 2) - (div.offsetHeight /2))+ "px";
					Element.setStyle(div, {top: tamanhoDivModificado});
				}else{
					Element.setStyle(div, {top: "0px"});
				}
			}
		}
	}
