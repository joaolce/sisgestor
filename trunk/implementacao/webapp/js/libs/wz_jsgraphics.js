	...
	this.fillPolygon = function(array_x, array_y)
	{
		...
		for(y = miny; y <= maxy; y++)
		{
			...
			for(i = 0; i < n; i++)
			{
				...
				if((y >= y1) && (y < y2)){polyInts[ints++] = Math.round((y-y1) * (x2-x1) / (y2-y1) + x1);}else if((y == maxy) && (y > y1) && (y <= y2)){polyInts[ints++] = Math.round((y-y1) * (x2-x1) / (y2-y1) + x1);}
			}
...
text both horizontally (e.g. right) and vertically within that rectangle */
	...
	this._mkArcDiv = function(x, y, oy, cx, cy, iOdds, aBndA, aBndZ, iSects)
	{
		...
		else
		{
			if(iSects & 0x00ff)
			{
				if(iSects & 0x02){xl = Math.max(x, aBndZ[y]);} else {xl = x;}
				if(iSects & 0x01){xr = Math.min(xrDef, aBndA[y]);} else {xr = xrDef;}
				y2 = cy - y - h;
				w = xr - xl;
				if(w > 0){this._mkDiv(xl, y2, w, h);}
			}
			if(iSects & 0xff00)
			{
				if(iSects & 0x0100){xl = Math.max(x, aBndA[y]);} else {xl = x;}
				if(iSects & 0x0200){xr = Math.min(xrDef, aBndZ[y]);} else {xr = xrDef;}
				y2 = cy + y + (iOdds >> 16);
				w = xr - xl;
				if(w > 0){this._mkDiv(xl, y2, w, h);}
			}
